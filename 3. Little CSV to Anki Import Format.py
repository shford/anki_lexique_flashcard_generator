...
def format_noun_declension(lemme, lemme_df, pos):
    ...
    df_stats = Row_Stats(rows=lemme_df)
    ...
    if pos in {'ver', 'adv', 'pre', 'con', 'ono'}:
        ...
        return format_noun_declension_nom(lemme_df, df_stats, lemme, pos, c_lemme_male, c_lemme_fem)
    elif 'adj' in pos:
        return format_noun_declension_adj(lemme_df, df_stats, lemme, pos, c_lemme_male, c_lemme_fem)

    # if no rule matched
    return None


def handle_hard_coded_formats(rows, lemme, c_male, c_fem):
    """
    ...
    if lemme in HARD_CODED_ADJ_4_ROWS:
        if lemme == 'tout':
            ...
            return sp_bold('quelque', 'adj', 'quelques')
        else:
            return False

    # exception because singular ortho is missing from lexique
    if lemme == 'fois':
        return sp_bold(lemme, 'nom', lemme, c_male, c_fem, 'f')

    # exceptions for unique/archaic/rare poetic spellings (that can't be corrected easily w/ rules)
    if lemme == 'oeil':
        return sp_bold(lemme, 'nom', 'yeux', c_male, c_fem, 'm')
    elif lemme == 'lieu':
        return sp_bold(lemme, 'adj', 'lieux')
    elif lemme == 'aïeul':
...
def singular_bold(lemme, pos, c_male=None, c_fem=None, genre=None) -> str:
    """
    :return: formatted string for one singular
    """
    if pos == 'nom':
        if c_male is None or c_fem is None or genre is None:
...
def ms_fs_bold(lemme, pos, c_male, ortho_ms, ortho_fs) -> str:
    """
...
def sp_bold(lemme, pos, ortho_p, c_male=None, c_fem=None, genre=None) -> str:
    """
...
def mpf_det_bold(lemme, pos, c_male, ortho_p, ortho_f) -> str:
    """
    ...
    if pos == 'nom':
        ...
        return f"<b>{lemme}</b> [<gr><i>m. </i></gr><blue>{c_male}</blue>; <gr><i>pl. </i></gr><blue>{ortho_p}</blue>; <gr><i>f. </i></gr><red>{ortho_f}</red>]"
...
def format_noun_declension_nom(rows, row_stats, lemme, pos, c_male, c_fem):
    rs = row_stats
    ...
    try:
        ...
        if len(rows) == 1:
            ...
            if rs.num_p == 1:
                ...
                if rs.num_genre_na == 1:
                    ...
                    #   genre   nombre
                    #     _      (s|_)
                    return singular_bold(lemme, 'adj')
                elif rs.num_s == 1:
                    # assume only a singular form exists
                    #   genre   nombre
                    #   (m|f)     s
                    return singular_bold(lemme, pos, c_male, c_fem, genre)
                else:
                    ...
                    #   genre   nombre
                    #   (m|f)     _
                    return sp_bold(lemme, pos, ortho, c_male, c_fem, genre)
        ...
        # two row cases
            ...
            # just reduce duplicate code for easy plural
            ortho_p = None
            if rs.num_p == 1:
                ortho_p = rs.rows_p.iloc[0]['ortho']

            if rs.num_m_s == 1 and rs.num_f_s == 1:
                return ms_fs_bold(lemme, pos, c_male, rs.rows_m_s.iloc[0]['ortho'], rs.rows_f_s.iloc[0]['ortho'])
            ...
            if pos == 'adj':
                if rs.num_genre_na == 2 and rs.num_s == 1 and rs.num_p == 1:
                    ...
                    #     _       s
                    #     _       p
                    return sp_bold(lemme, pos, ortho_p)
                elif rs.num_genre_na == 2 and rs.num_nombre_na == 2:
                    #   genre   nombre
                    #     _       _
                    #     _       _
                    return sp_bold(lemme, pos, row2['ortho']) # gamble that the corpus row order is correct

                # regular formatting
                if rs.num_p == 1:
                    return sp_bold(lemme, pos, ortho_p)
                else:
                    return sp_bold(lemme, pos, row2['ortho'])
            ...
            if pos == 'nom':
                ...
                if rs.num_genre_na == 2 and rs.num_s == 1 and rs.num_p == 1:
                    ...
                    Ahh french, there's just this: Je ne sais pas.
                     ...
                     There's a surprising amount of these. Thanks French. 
                    """
                    return [sp_bold(lemme, pos, ortho_p, c_male, c_fem, 'm'), sp_bold(lemme, pos, ortho_p, c_male, c_fem, 'f')]

                # apparently nouns with conflicting genres across different plurality are rare, but valid and modern.. boof
                if rs.num_m_s == 1 and rs.num_f_p == 1:
                    #   genre   nombre
                    #     m       s
                    #     f       p
                    return sp_bold(lemme, pos, ortho_p, c_male, c_fem, 'ms_fp')
                if rs.num_f_s == 1 and rs.num_m_p == 1:
                    #   genre   nombre
                    #     f       s
                    #     m       p
                    return sp_bold(lemme, pos, ortho_p, c_male, c_fem, 'fs_mp')

                # regular formatting for male
                if rs.num_f == 0 and rs.num_m >= 1 and rs.num_m_p == 1:
                    if rs.num_p == 1:
                        return sp_bold(lemme, pos, ortho_p, c_male, c_fem, 'm')
                    else:
                        return sp_bold(lemme, pos, row2['ortho'], c_male, c_fem, 'm')

                # regular formatting for fem.
                if rs.num_m == 0 and rs.num_f >= 1:
                    if rs.num_p == 1:
                        return sp_bold(lemme, pos, ortho_p, c_male, c_fem, 'f')
                    else:
                        return sp_bold(lemme, pos, row2['ortho'], c_male, c_fem, 'f')

        # my voluntary & entirely avoidable suffering is your flashcards
        elif len(rows) == 3:
            return noun_three(rows, row_stats, lemme, pos, c_male, c_fem)

        elif len(rows) == 4:
            return noun_four(rows, row_stats, lemme, pos, c_male, c_fem)

        elif len(rows) == 5:
            return noun_five(rows, row_stats, lemme, pos)
...
def format_noun_declension_adj(rows, row_stats, lemme, pos, c_lemme_male, c_lemme_fem):
    rs = row_stats
    ...
    # if two, three, or five rows, apply same rules as 'nom'
    elif len(rows) == 2 or len(rows) == 3 or len(rows) == 5:
        result = format_noun_declension_nom(rows, row_stats, lemme, pos, c_lemme_male, c_lemme_fem)
...
def noun_three(rows, row_stats, lemme, pos, c_male, c_fem):
    rs = row_stats
    ...
    if pos == 'nom':
        ...
        if rs.num_p == 3 and (rs.num_m == 3 or rs.num_f == 3 or rs.num_genre_na == 3):
            ...
            if rs.num_f_s == 1 and rs.num_na_s == 1 and rs.num_na_p == 1:
                ...
                #     f       s
                #     _       p
                return mpf_det_bold(lemme, pos, c_male, rs.rows_na_p.iloc[0]['ortho'], rs.rows_f_s.iloc[0]['ortho'])
            elif rs.num_m_s == 1 and rs.num_na_s == 1 and rs.num_na_p == 1:
                #   genre   nombre
                #     m       s
                #     _       s
                #     _       p
                return mpf_det_bold(lemme, pos, c_male, rs.rows_na_p.iloc[0]['ortho'], rs.rows_na_s.iloc[0]['ortho'])
            ...
            if rs.num_m == 3 or rs.num_f == 3:
                ...
                if '-' in lemme:
                    ...
                    if len(p_row1) > len(p_row2):
                        return sp_bold(lemme, pos, p_row1, c_male, c_fem, genre)
                    elif len(p_row2) > len(p_row1):
                        return sp_bold(lemme, pos, p_row2, c_male, c_fem, genre)
                    else:
                    ...
                    if p_row2[-1:] == 's' and p_row1[-1:] != 's':
                        return sp_bold(lemme, pos, p_row2, c_male, c_fem, genre)
            elif rs.num_m_s == 1 and rs.num_m_p == 1 and rs.num_f_p == 1:
    ...
    if ortho_p != ortho_m and ortho_f and ortho_f != ortho_m:
        return mpf_det_bold(lemme, pos, c_male, ortho_p, ortho_f)
...
def noun_four(rows, row_stats, lemme, pos, c_lemme_male, c_lemme_fem):
    rs = row_stats
        ...
        if groups[key] is not None:
            ...
            if rs.num_m_s == 1 and rs.num_f_s == 2 and rs.num_m_p == 1 and rs.num_f_p == 0:
                ...
                if row_fs1[-len(modern_suffix):] == modern_suffix:
                    return mpf_det_bold(lemme, pos, c_lemme_fem, rs.rows_m_p.iloc[0]['ortho'], row_fs1)
                elif row_fs2[-len(modern_suffix):] == modern_suffix:
                    return mpf_det_bold(lemme, pos, c_lemme_fem, rs.rows_m_p.iloc[0]['ortho'], row_fs2)
                else:
                    return None
...
def noun_five(rows, row_stats, lemme, pos):
    rs = row_stats
